          ...
          : Column(children: [
              ...
              Expanded(
                child: TabBarView(
                  ...
                  children: [
                    _DonneesPersonnellesTab(user: _user!, onRefresh: _load, onLogout: _logout),
    ...
    return Container(
      ...
      child: Row(children: [
        CircleAvatar(
          ...
          backgroundImage: avatarPath != null ? NetworkImage('${ApiService.baseUrl}$avatarPath') : null,
          child: avatarPath == null ? const Icon(Icons.person, color: AppColors.blue, size: 30) : null,
        ),
        ...
        Expanded(
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text(
              _user?.fullName ?? '',
              style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: AppColors.blueLight),
            ),
          ]),
        ),
        IconButton(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          if (_mesFavoris.isEmpty)
            _AlertBanner(
              ...
              title: 'Aucun favori',
              message: "Vous n'avez pas encore ajouté de ressource à vos favoris. Explorez les ressources disponibles pour en ajouter.",
            )
          else
            ListView.separated(
              shrinkWrap: true,
...
class _DonneesPersonnellesTab extends StatefulWidget {
  ...
  const _DonneesPersonnellesTab({required this.user, required this.onRefresh, required this.onLogout});
  @override
...
class _DonneesPersonnellesTabState extends State<_DonneesPersonnellesTab> {
  ...
  Future<void> _fetchRegions() async {
    ...
    try {
      final res = await http.get(Uri.parse('https://geo.api.gouv.fr/regions?fields=nom'));
      if (res.statusCode == 200) {
        final data = jsonDecode(res.body) as List<dynamic>;
        final list = data.map((e) => (e as Map<String, dynamic>)['nom'] as String).toList()
          ..sort((a, b) => a.compareTo(b));
        if (mounted) setState(() { _regions = list; _loadingRegions = false; });
      } else {
        if (mounted) setState(() { _regions = _fallbackRegions; _loadingRegions = false; });
      }
    } catch (_) {
      if (mounted) setState(() { _regions = _fallbackRegions; _loadingRegions = false; });
    }
  ...
  Future<void> _pickPhoto() async {
    final xfile = await ImagePicker().pickImage(source: ImageSource.gallery, maxWidth: 2048, imageQuality: 90);
    ...
    setState(() { _photoLocal = file; _apiError = null; });
    final session = ApiService.session;
    if (session == null) return;
    try {
  ...
  Future<void> _removePhoto() async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() { _photoUrl = null; _photoLocal = null; });
      widget.onRefresh();
    } catch (_) {
      if (!mounted) return;
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() => _successMessage = 'Vos informations ont été mises à jour avec succès.');
      widget.onRefresh();
    } catch (e) {
      if (!mounted) return;
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Row(children: [
              CircleAvatar(
                ...
                child: photoProvider == null ? const Icon(Icons.person, color: AppColors.blue, size: 32) : null,
              ),
            ...
            const SizedBox(height: 16),
            // ── Champs ──────────────────────────────────────────────────────
            _EditFieldWithError(label: 'Prénom', controller: _prenomCtrl, errorText: _prenomError),
            const SizedBox(height: 12),
            _EditFieldWithError(label: 'Nom', controller: _nomCtrl, errorText: _nomError),
            const SizedBox(height: 12),
            _EditFieldWithError(
            ...
            const SizedBox(height: 12),
            _EditFieldWithError(label: 'Téléphone', controller: _telCtrl, type: TextInputType.phone),
            const SizedBox(height: 12),
            _EditFieldWithError(label: "Phrase d'accroche", controller: _phraseAccrocheCtrl),
            const SizedBox(height: 12),
            _EditFieldWithError(label: 'Description', controller: _descCtrl, maxLines: 4),
            const SizedBox(height: 12),
            // ── Région ──────────────────────────────────────────────────────
            Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              if (_loadingRegions)
                const SizedBox(
                  height: 44,
                  child: Center(child: CircularProgressIndicator(strokeWidth: 2, color: AppColors.blueLight)),
                )
              else
                Container(
                  ...
                  child: DropdownButton<String>(
                    ...
                    hint: const Text('Sélectionnez une région', style: TextStyle(fontSize: 13, color: AppColors.grey)),
            ...
            // ── Alertes ─────────────────────────────────────────────────────
            if (_apiError != null) ...[
              _AlertBanner(type: _BannerType.error, title: 'Mise à jour impossible', message: _apiError!),
              const SizedBox(height: 12),
            ],
            if (_successMessage != null) ...[
              _AlertBanner(type: _BannerType.success, title: 'Profil mis à jour', message: _successMessage!),
              const SizedBox(height: 12),
            ],
            // ── Bouton Enregistrer ───────────────────────────────────────────
            SizedBox(
...
class _PrefSwitchState extends State<_PrefSwitch> {
  ...
  Widget build(BuildContext context) => Row(
        ...
        children: [
          ...
          Switch(value: _value, onChanged: (v) => setState(() => _value = v), activeThumbColor: AppColors.blueLight),
        ],
      );
}
        ...
        TextField(
          ...
          decoration: InputDecoration(
            ...
            enabledBorder: OutlineInputBorder(
              borderRadius: BorderRadius.circular(8),
              borderSide: BorderSide(color: errorText != null ? AppColors.red : AppColors.border),
            ),
            focusedBorder: OutlineInputBorder(
              borderRadius: BorderRadius.circular(8),
              borderSide: BorderSide(color: errorText != null ? AppColors.red : AppColors.blueLight, width: 1.5),
            ),
...
class _AlertBanner extends StatelessWidget {
  ...
  const _AlertBanner({required this.type, required this.title, required this.message});
  ...
  Widget build(BuildContext context) => Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(title, style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: _border)),
          const SizedBox(height: 4),
...
class _ProfileAside extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        ...
        child: Column(children: [
          ...
          const SizedBox(height: 12),
          if (email.isNotEmpty)
            _AsideRow(icon: Icons.mail_outline, text: email),
          ...
          SizedBox(
            ...
            child: OutlinedButton.icon(
              ...
              label: const Text('Se déconnecter', style: TextStyle(color: AppColors.red, fontSize: 13)),
              style: OutlinedButton.styleFrom(
...
class _AsideRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Padding(
        ...
        child: Row(children: [
          ...
          Expanded(child: Text(text, style: AppText.body, overflow: TextOverflow.ellipsis)),
        ]),
      );
}
...
class _FavoriRichCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final categorie = (r['categorie'] as Map<String, dynamic>?)?['nom'] as String?;
    ...
    final dateAjout = favori['dateAjoutFavori'] != null
        ? DateFormat('dd/MM/yyyy').format(DateTime.parse(favori['dateAjoutFavori'] as String))
        : null;
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(titre, style: AppText.h3, maxLines: 2, overflow: TextOverflow.ellipsis),
        if (description != null && description.isNotEmpty) ...[
          const SizedBox(height: 6),
          Text(description, style: AppText.body, maxLines: 3, overflow: TextOverflow.ellipsis),
        ],
        ...
        if (auteur != null || dateAjout != null) ...[
          ...
          Row(children: [
            if (auteur != null) ...[
              const Icon(Icons.person_outline, size: 13, color: AppColors.grey),
              const SizedBox(width: 4),
              Text(auteur, style: AppText.caption),
              const SizedBox(width: 12),
            ],
            if (dateAjout != null) ...[
              const Icon(Icons.calendar_today, size: 13, color: AppColors.grey),
              const SizedBox(width: 4),
...
class _Chip extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        ...
        child: Text(label, style: const TextStyle(fontSize: 11, color: AppColors.blueLight, fontWeight: FontWeight.w600)),
      );
}
